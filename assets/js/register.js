document.addEventListener("DOMContentLoaded", () => {
  ...
  const token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2ODE4NGUxZDVlZjNhNDAwMTVlYWMwMTMiLCJpYXQiOjE3NDY0MjMzMjUsImV4cCI6MTc0NzYzMjkyNX0.sjewwilLks-3cdgn_4ZfGk6Rlcff_qG0LGj3dQlNKHg";
  ...
  if (productId) {
    fetch(endpoint + productId, {
      ...
      .then((data) => {
        ...
        document.getElementById("surname").value = data.description;
        document.getElementById("mail").value = data.brand;
        document.getElementById("imageUrl").value = data.imageUrl;
        document.getElementById("pincode").value = data.price;
        deleteBtn.classList.remove("d-none");
        updateImagePreview(data.imageUrl);
      })
      .catch((err) => showAlert("Errore nel caricamento account", "danger"));
  }
  ...
  form.addEventListener("submit", function (e) {
    ...
    fetch(url, {
      ...
      .then(() => {
        showAlert(productId ? "Account modificato con successo!" : "Account creato con successo!", "success");
        if (!productId) {
          form.reset();
